...
export default function List(props) {
  ...
  async function GetSearches() {
    const newSearchesReport = reports.map(report => ({
      _id: report.patientId,
      specialistReq: report.specialistReq,
      name: report.patient
  }));
  const newSearchpatient = patients.map(patient => ({
    _id: patient._id,
    name : patient.name,
  }))
  const combinedLists = [...newSearchpatient, ...newSearchesReport];
  console.log(combinedLists)
  setSearches(combinedLists);
  ...
  useEffect(() => {
    if (searchTerm) {
      const filtered = searches.filter(item => 
        ...
        (item.specialistReq && item.specialistReq.toLowerCase().includes(searchTerm.toLowerCase()))
      );
  ...
  const handleSearchClick = (item) => {
    if (item.name) {
      handleModify(item._id)();
    } else if (item.specialistReq) {
      handleModify(item._id)();
    }
    setSearchTerm('');
    setDropdownVisible(false);
  ...
  const filteredReports = showVerifiedReports
    ...
    return reports && patients && searches && (
              ...
              {dropdownVisible && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg">
                  {filteredSearches.map((item, index) => (
                    ...
                    >
                      {item.name ? item.name : item.specialistReq}
                    </div>
                  ))}
                </div>
              )}
